...
// ---------- Animation creator ----------
const createAnimation = (variant, start = 'center', blur = false, url) => {
...
export const useThemeToggle = ({
  ...
  // Apply initial theme from localStorage / system
  useEffect(() => {
  ...
  // Inject dynamic CSS for the current animation
  ...
  // Actual theme change (class + localStorage)
  ...
  const toggleTheme = useCallback(() => {
    ...
    // Generate animation CSS for the current direction
    const animation = createAnimation(variant, start, blur, gifUrl);
    updateStyles(animation.css);

    const switchTheme = () => applyTheme(newIsDark);
  ...
  }, [isDark, variant, start, blur, gifUrl, updateStyles, applyTheme]);

  const setCrazyLightTheme = useCallback(() => {
    setIsDark(false);
    const animation = createAnimation(variant, start, blur, gifUrl);
    updateStyles(animation.css);
    const switchTheme = () => applyTheme(false);
    if (!document.startViewTransition) {
      switchTheme();
      return;
    }
    document.startViewTransition(switchTheme);
  }, [variant, start, blur, gifUrl, updateStyles, applyTheme]);

  const setCrazyDarkTheme = useCallback(() => {
    setIsDark(true);
    const animation = createAnimation(variant, start, blur, gifUrl);
    updateStyles(animation.css);
    const switchTheme = () => applyTheme(true);
    if (!document.startViewTransition) {
      switchTheme();
      return;
    }
  ...
  return { isDark, setIsDark, toggleTheme, setCrazyLightTheme, setCrazyDarkTheme };
};
...
export const ThemeToggleButton = ({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn('size-10 cursor-pointer p-0 transition-all duration-300 active:scale-95', className)}
      onClick={toggleTheme}
