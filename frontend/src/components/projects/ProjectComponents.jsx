...
import Prisma from '../technologies/Prisma.jsx';
...
const TechStack = ({ technologies }) => (
  <div className="bg-muted/20 my-6 rounded-lg border p-4">
    ...
    <div className="flex flex-wrap gap-2">
      {technologies.map((tech) => <Technology key={tech} name={tech} />)}
    </div>
  </div>
);
...
const ProjectMeta = ({ timeline, role, team, status }) => (
  <div className="bg-muted/20 my-6 grid gap-4 rounded-lg border p-4 sm:grid-cols-2 lg:grid-cols-4">
    {timeline && <div><h5 className="text-muted-foreground text-sm font-semibold">Timeline</h5><p className="text-sm">{timeline}</p></div>}
    {role && <div><h5 className="text-muted-foreground text-sm font-semibold">Role</h5><p className="text-sm">{role}</p></div>}
    {team && <div><h5 className="text-muted-foreground text-sm font-semibold">Team</h5><p className="text-sm">{team}</p></div>}
    {status && (
      <div>
...
const Challenges = ({ challenges }) => (
  <div className="my-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-950/20">
    <h4 className="mb-3 text-lg font-semibold text-yellow-800 dark:text-yellow-200">Key Challenges</h4>
...
export const ProjectComponents = {
  img: ({ src, alt, ...props }) => (
    <img src={src} alt={alt} width={800} height={400} className="rounded-lg w-full" {...props} />
  ),
  h1: ({ children, ...props }) => <h1 className="mb-6 text-4xl font-bold" {...props}>{children}</h1>,
  h2: ({ children, ...props }) => <h2 className="mt-8 mb-4 text-3xl font-semibold" {...props}>{children}</h2>,
  h3: ({ children, ...props }) => <h3 className="mt-6 mb-3 text-2xl font-medium" {...props}>{children}</h3>,
  p: ({ children, ...props }) => <p className="text-muted-foreground mb-4 leading-7" {...props}>{children}</p>,
  ul: ({ children, ...props }) => <ul className="mb-4 ml-6 list-disc space-y-2" {...props}>{children}</ul>,
  ol: ({ children, ...props }) => <ol className="mb-4 ml-6 list-decimal space-y-2" {...props}>{children}</ol>,
  li: ({ children, ...props }) => <li className="text-muted-foreground leading-7" {...props}>{children}</li>,
  pre: ({ children, ...props }) => {
    const getTextContent = (node) => {
      if (typeof node === 'string') return node;
      if (typeof node === 'number') return String(node);
      if (isValidElement(node) && node.props?.children) return getTextContent(node.props.children);
  ...
  code: ({ children, className, ...props }) => {
    ...
    return <code className="rounded px-2 py-1 font-mono text-sm" {...props}>{children}</code>;
  },
  blockquote: ({ children, ...props }) => (
    <blockquote className="border-primary text-muted-foreground mb-4 border-l-4 pl-4 italic" {...props}>{children}</blockquote>
  ),
  Technology,
  TechStack,
  ProjectMeta,
