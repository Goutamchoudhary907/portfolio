...
export function ProjectContent({ frontmatter, Component }) {
  const {
    title, description, image, technologies,
    github, live, timeline, role, team,
    status, challenges, learnings,
  } = frontmatter;
  ...
  return (
    <article className="mx-auto max-w-4xl">
      ...
      <header className="mb-8 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            {live && (
              <Button asChild>
                <a href={live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
                  <Website className="size-4" /> Live Demo
                </a>
              </Button>
            )}
            {github && (
      ...
      {/* MDX Content — rendered directly as a React component */}
      <div className="prose prose-neutral dark:prose-invert max-w-none">
        {Component ? (
          <Component components={ProjectComponents} />
        ) : (
          <p className="text-muted-foreground">No case study content available.</p>
        )}
      </div>
    </article>
  );
